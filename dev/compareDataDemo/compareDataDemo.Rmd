...
```{r master20, message=FALSE, warning=FALSE}
master20 <- readr::read_csv("data/csv/m20.csv")
```
...
gg_names <- sort(
  str_to_title(
    str_replace_all(names(changed$num_diffs), "_", " ")
    ), 
  decreasing = TRUE)
gg_names_alpha_p1 <- gg_names[1]
gg_names_alpha_p2 <- gg_names[2]
# create graph
num_diffs_graph <- ggplot2::ggplot(data = changed$num_diffs, 
...
Click on the **Code** button to view the `uploadDataUI()` function.
...
Click on the **Code** button to view the `uploadDataServer()` function.
...
1.  [SelectDataDemo](https://mjfrigaard.shinyapps.io/selectDataDemo/): after uploading data into the application, this module can further refine the data used in the comparison (*optional*).

## UI

Click on the **Code** button to view the `selectDataUI()` function:
...
Click on the **Code** button to view the `selectDataServer()` function:
...
Click on the **Code** button to view the `compareDataUI()` function:
...
Click on the **Code** button to view the `compareDataServer()` function:
