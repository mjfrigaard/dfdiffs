...
- Application themes
  - `dfdiffs_fresh_theme()` is built using the `fresh::create_theme()`   
...
We define the `dfdiffs_fresh_theme()` using `fresh::create_theme()`. 

```{r dfdiffs_fresh_theme}
...
The base color will use the `#761763` background color (based on the  violet (`#a9218e`) :
...
The compare color will use the `#2f3688` background color (based on the  purple (`#353d98`) :
...
The `create_join_column()` function creates a new bs4Dash::column from existing columns. 
...
In the return list, a **`data_source`** bs4Dash::column is added to each dataset, and it's value is the name the user provided for each dataset (see list below):
