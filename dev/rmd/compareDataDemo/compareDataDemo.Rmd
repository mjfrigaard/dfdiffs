...
change_row_by_row <- create_changed_data(compare = master20, master15, 
                                  cols = c("playerID", "nameFirst", 
                                           "nameLast", "nameGiven", 
                                           "birthCity"))
change_single_id  <- create_changed_data(compare = master20, master15,
                                  by = "playerID", 
                                  by_col = "join_column",
                                  cols = c("nameFirst", "nameLast",
                                           "nameGiven", "birthCity"))
change_multiple_ids <- create_changed_data(compare = master20, master15,
...
If `create_changed_data()` is made using a row-by-row comparison, these differences are joined to the compare data (`master20`) on the `compare` column. 

First we add a `rownumber` column to `master20`
...
```{r compare_single_id}
compare_single_id <- create_join_column(df = master20, 
                   by_colums = "playerID", 
...
compare_multiple_ids <- create_join_column(df = master20, 
