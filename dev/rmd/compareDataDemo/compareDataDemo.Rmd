...
If no `by` bs4Dash::column is provided, these are the only two tables returned.

```{r names-change_row_by_row}
names(change_row_by_row)
```

### Single `by` bs4Dash::column comparisons 

A single `by` bs4Dash::column will return four tables,

```{r names-change_single_id}
names(change_single_id)
```

### Multiple `by` bs4Dash::column comparisons 
...
The `num_diffs_graph` graph is a bs4Dash::column graph of all the variables with differences.
...
If `create_changed_data()` is made using a row-by-row comparison, these differences are joined to the compare data (`master22`) on the `compare` bs4Dash::column. 

First we add a `rownumber` bs4Dash::column to `master22`
...
The `uploadData` module 'passes' the data to the `selectData` module, which displays the data for the user to review and, if necessary, reduce the number of columns and designate a join bs4Dash::column (or columns). The `uploadData` module can import two data files, but they won't be passed to the `selectData` module without an accompanying name. 
...
Each of these reactive datasets contain 1) the selected columns for comparison, 2) the designated join bs4Dash::column(s), and the following metadata:

-   `data_source`: this is the name we designated in the `uploadData` module.

-   `join_source`: the name of the bs4Dash::column(s) used for creating a unique observation between the two datasets. 
...
The diffs table contains the modified variable (`Variable Name`), the joining bs4Dash::column (`playerID`), and the modified values (`comapre Value` & `base Value`).
