...
These data do not have a unique identifier, but we can create one with the `create_new_column()` function. 
...
We'll run this on two datasets: 
...
User supplies a `by`/`keys` column, and a new column name. This is accomplished with the `rename_join_col()` (see example below).
...
And run this condition using `master15` vs. `master20`
...
```{r cond-8-1-create_changed_data, eval=FALSE}
