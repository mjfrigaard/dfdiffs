...
The `SAEDESC1` columns have a lot of unstructured text with multiple commas, so we'll use this as a test to separate the column names from the `value_changes` and `newvalues` columns. 
...
Now we test these using the regular expression pattern. 
...
Now that we have a regular expression for matching the column names in the `value_changes`, we'll get a count of each regular expression occurrence. 
...
We can derive the `Field` and `ChangeCount` from the imported `snap_ae_raw` from the spreadsheet. 
...
We can build a graph of these changes, but we have to manually set the font size because the long `y` axis. 
...
Then we set names to these sheets in the vector, `form_sheets_nmd`
...
We can now import all the sheets in the file at the end of the path in `snapshot_pth` and store it in `all_sheets`. 
...
If we check the names of this list, we see they are all the imported sheets. 
...
If we `map()` the `count_change_columns()` function to all items in the `all_field_counts` list.
...
We can check this with various forms. 
