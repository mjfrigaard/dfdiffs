...
Within `dfdiffs`, `create_modified_data()` calls `rename_join_col()` and `create_new_column()` to build (and name) the join column; the comparison itself comes from `arsenal::comparedf()` (from another package, so it isn't shown below). The call tree below was generated from the package source with [stackcallr](https://github.com/mjfrigaard/stackcallr) (`pak::pak("mjfrigaard/stackcallr")`); only functions defined in `dfdiffs` are shown.
