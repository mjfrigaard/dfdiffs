...
create_deleted_data <- function(compare, base, by = NULL, by_col = NULL, cols = NULL) {
  ...
  if (is.null(by) & is.null(by_col) & is.null(cols)) {
    ...
    compare_cols <- select(compare, tidyselect::all_of(cols))
    base_cols <- select(base, tidyselect::all_of(cols))
    ...
    # no 'by_col'
    compare_cols <- dplyr::select(compare, tidyselect::all_of(by), tidyselect::all_of(cols))
    base_cols <- dplyr::select(base, tidyselect::all_of(by), tidyselect::all_of(cols))
    deleted_join <- dplyr::anti_join(x = base_cols, y = compare_cols,
                                    by = dplyr::intersect(
                                    x = names(base_cols),
                                    y = names(compare_cols)))
    ...
    compare_join <- create_new_column(data = compare,
                      cols = tidyselect::all_of(by), new_name = {{by_col}})
    compare_cols <- dplyr::select(compare_join, {{by_col}}, tidyselect::all_of(cols))
    base_cols <- dplyr::select(base_join, {{by_col}}, tidyselect::all_of(cols))
    deleted_join <- dplyr::anti_join(x = base_cols, y = compare_cols,
                                      by = dplyr::intersect(
                                      x = names(base_cols),
                                      y = names(compare_cols)))
    deleted_data <- dplyr::distinct(deleted_join)
      ...
      compare_cols <- create_new_column(data = compare,
                        cols = tidyselect::all_of(by), new_name = "join")
      base_join <- dplyr::select(base_cols, tidyselect::matches("join"), tidyselect::all_of(cols))
      compare_join <- dplyr::select(compare_cols, tidyselect::matches("join"), tidyselect::all_of(cols))
      deleted_join <- dplyr::anti_join(x = base_join, y = compare_join,
                                    by = dplyr::intersect(
                                      x = names(base_join),
                                      y = names(compare_join)))
    ...
    # 10) multiple 'by', new column ('by_col'), multiple compare 'cols' -----
      ...
      base_cols <- create_new_column(data = base,
                        cols = tidyselect::all_of(by), new_name = {{by_col}})
      compare_join <- dplyr::select(compare_cols, {{by_col}}, tidyselect::all_of(cols))
      base_join <- dplyr::select(base_cols, {{by_col}}, tidyselect::all_of(cols))
      deleted_join <- dplyr::anti_join(x = base_join, y = compare_join,
                                        by = dplyr::intersect(
                                          x = names(base_join),
                                          y = names(compare_join)))
